...
export namespace vox
{
    export namespace texture
    {
        export class BytesTextureProxy extends RawDataTextureProxy
        {
            ...
            uploadFromBytes(bytes:Uint8Array, miplevel:number = 0,imgWidth:number = -1,imgHeight:number = -1,offsetx:number = 0,offsety:number = 0,rebuild:boolean = false):void
            {
                super.uploadFromBytes(bytes, miplevel, imgWidth, imgHeight, offsetx, offsety, rebuild);
            }
            updateSubBytes(bytes:Uint8Array,px:number,py:number,twidth:number,theight:number, miplevel:number = 0):void
            {
