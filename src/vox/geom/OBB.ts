...
class OBB implements IOBB {
	...
	toString(): string {
		return "[OBB(min->" + this.min + ",size(" + this.m_width + "," + this.m_height + "," + this.m_long + "))]";
	}
	...
	intersectRL(ltv: Vector3D, lpv: Vector3D): boolean {
		...
		//console.log("OBB::IntersectRL uses...");
		if (tmin > tmax) {
			f = tmax;
			tmax = tmin;
			tmin = f;
	...
	static IntersectionRL1(ltv: Vector3D, lpv: Vector3D, ab: OBB, outV: Vector3D): boolean {
		let f: number = 0;
	...
	// @param				ltv		射线的切向
	// @param				lpv		射线上的一点
	// @param				ab		updateFast() 过的 OBB 实例
	// @param				outV	存放交点的 Vector3D实例
	//
	static IntersectionRL2(ltv: Vector3D, lpv: Vector3D, ab: OBB, outV: Vector3D): boolean {
		// 计算包围球
