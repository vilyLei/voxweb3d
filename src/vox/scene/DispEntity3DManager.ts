...
import RPOUnit from "../render/RPOUnit";
...
export default class DispEntity3DManager {
	...
	removeEntity(entity: IRenderEntity): void {
		...
		if (display != null && display.__$$runit != null) {
			const re = (display.__$$runit as RPOUnit).rentity as IRenderEntity;
			...
			while (parent) {
				if (!parent.hasParent() && parent.getREType() >= 20) {
					...
					const rprocess = this.m_processBuider.getNodeByUid(parent.__$wprocuid) as RenderProcess;
				...
				if (po) {
					if (po.count > 0) {
						if (po.count < 2) {
							...
							let len = RCRPObj.RenerProcessMaxTotal;
							for (let i = 0; i < len; ++i) {
								if ((po.idsFlag & (1 << i)) > 0) {
	...
	addEntity(entity: IRenderEntity, processUid: number, deferred: boolean = false): boolean {
		if (entity != null) {
			...
			if (disp != null) {
				...
				entity.__$rseFlag = this.m_rprocess.getSortEnabled()
					? RSEntityFlag.AddSortEnabled(entity.__$rseFlag)
					: RSEntityFlag.RemoveSortEnabled(entity.__$rseFlag);
	...
	addContainer(container: IRenderEntityContainer, processUid: number): void {
		const rprocess = this.m_processBuider.getNodeByUid(processUid) as RenderProcess;
		rprocess.addContainer(container);
	}
	removeContainer(container: IRenderEntityContainer, processUid: number): void {
		const rprocess = this.m_processBuider.getNodeByUid(processUid) as RenderProcess;
