...
export default class Box3DEntity extends Default3DEntity {
    ...
    protected __activeMesh(material: IRenderMaterial): void {
        this.m_mh = this.getMesh() as Box3DMesh;
        if (this.m_mh == null) {
            this.m_mh = new Box3DMesh();
        }
        if (this.m_mh) {
            let mesh = this.m_mh;
            if (this.m_transMatrix) {
    ...
    setFaceUVSAt(i: number, uvslen8: Float32Array, offset: number = 0): void {
        if (this.m_mh) {
            this.m_mh.setFaceUVSAt(i, uvslen8, offset);
        }
    }
    transformFaceAt(i: number, mat4: Matrix4): void {
        if (this.m_mh) {
            this.m_mh.transformFaceAt(i, mat4);
        }
    }
    getFaceCenterAt(i: number, outV: Vector3D): void {
        if (this.m_mh) {
            this.m_mh.getFaceCenterAt(i, outV);
        }
    }
    destroy(): void {
        super.destroy();
