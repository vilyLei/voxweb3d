...
export namespace vox
{
    export namespace render
    {
        export class RenderColorMask
        {
            ...
            private static __s_state:number = -1;
            private static __s_states:RenderColorMask[] = [];
            private static __s_statesLen:number = 1;
            private static __stsMap:Map<number,RenderColorMask> = new Map();
            private static __stsNameMap:Map<string,RenderColorMask> = new Map();
            ...
            {
                this.m_uid = RenderColorMask.__s_uid++;
                this.m_rBoo = rBoo;
                this.m_gBoo = gBoo;
                this.m_bBoo = bBoo;
                this.m_aBoo = aBoo;
            ...
            use():void
            {
                if(RenderColorMask.__s_state != this.m_uid)
                {
                    RenderColorMask.Rstate.setColorMask(this.m_rBoo,this.m_gBoo,this.m_bBoo,this.m_aBoo);
                    RenderColorMask.__s_state = this.m_uid;
                }
            }
    
            static Create(objName:string,rBoo:boolean,gBoo:boolean,bBoo:boolean,aBoo:boolean):number
            {
                if(RenderColorMask.__stsNameMap.has(objName))
                {
                    let po:RenderColorMask = RenderColorMask.__stsNameMap.get(objName);
                    return po.getUid();
                }
                let key:number = (rBoo?1<<6:1<<5) | (gBoo?1<<4:1<<3) | (bBoo?1<<2:1<<1) | (aBoo?1:0);
            
                if(RenderColorMask.__stsMap.has(key))
                {
                    let po:RenderColorMask = RenderColorMask.__stsMap.get(key);
                    key = po.getUid();
                }
                else
                {
                    ...
                    key = po.getUid();
                    RenderColorMask.__stsMap.set(key, po);
                    RenderColorMask.__stsNameMap.set(objName, po);
                    RenderColorMask.__s_states.push(po);
                    ++RenderColorMask.__s_statesLen;
                }
                return key;
            }
            ...
            static GetColorMaskByName(objName:string):number
            {
                if(RenderColorMask.__stsNameMap.has(objName))
                {
                    let po = RenderColorMask.__stsNameMap.get(objName);
                    return po.getUid();
                }
                return -1;
            }
            ...
            {
                if(RenderColorMask.__s_unlocked && RenderColorMask.Rstate.roColorMask != state)
                {
                    if(state > -1 && state < RenderColorMask.__s_statesLen)
                    {
                        RenderColorMask.__s_states[state].use();
                    }
                }
            }
            ...
            static Lock():void
            {
                RenderColorMask.__s_unlocked = false;
            }
            static Unlock():void
            {
                RenderColorMask.__s_unlocked = true;
            }
            static Reset():void
            {
                RenderColorMask.__s_state = -1;
            }
        }
        ...
        export class RenderStateObject
        {
            private static __s_uid:number = 0;
            private static __s_state:number = -1;    
            private static __s_states:RenderStateObject[] = [];
            private static __s_statesLen:number = 1;
            private static __s_blendMode:number = -1;
            private static __s_depthTestMode:number = -1;
            private static __stsMap:Map<number,RenderStateObject> = new Map();
            private static __stsNameMap:Map<string,RenderStateObject> = new Map();
            private static __s_unlocked:boolean = true;
            ...
            {
                this.m_uid = RenderStateObject.__s_uid++;
            ...
            use():void
            {
                if(RenderStateObject.__s_state != this.m_uid)
                {
                    ...
                    if(RenderStateObject.__s_blendMode < 0)
                    {
                        RenderStateObject.Rstate.setBlendMode(this.m_blendMode);
                    }
                    else
                    {
                        RenderStateObject.Rstate.setBlendMode(RenderStateObject.__s_blendMode);
                    }
                    //RenderStateObject.Rstate.setDepthTestMode(this.m_depthTestMode);
                    if(RenderStateObject.__s_depthTestMode < 0)
                    {
                        RenderStateObject.Rstate.setDepthTestMode(this.m_depthTestMode);
                    }
                    else
                    {
                        RenderStateObject.Rstate.setDepthTestMode(RenderStateObject.__s_depthTestMode);
                    }
                    //
                    RenderStateObject.__s_state = this.m_uid;
                }
            }
            static Create(objName:string,cullFaceMode:number,blendMode:number,depthTestMode:number):number
            {
                if(RenderStateObject.__stsNameMap.has(objName))
                {
                    let po = RenderStateObject.__stsNameMap.get(objName);
                    return po.getUid();
                }
                let key = depthTestMode << 8 | blendMode<<4 | cullFaceMode;
                if(RenderStateObject.__stsMap.has(key))
                {
                    let po = RenderStateObject.__stsMap.get(key);
                    key = po.getUid();
                }
                else
                {
                    ...
                    key = po.getUid();
                    RenderStateObject.__stsMap.set(key, po);
                    RenderStateObject.__stsNameMap.set(objName, po);
                    RenderStateObject.__s_states.push(po);
                    ++RenderStateObject.__s_statesLen;
                }
                return key;
            }
            ...
            {
                //if(RenderStateObject.__s_unlocked && RenderStateObject.Rstate.roState != state)                
                if(RenderStateObject.__s_unlocked && RenderStateObject.__s_state != state)
                {
                    if(state > -1 && state < RenderStateObject.__s_statesLen)
                    {
                        RenderStateObject.__s_states[state].use();
                    }
                }
            }
            static UseRenderStateByName(stateName:string):void
            {
                if(RenderStateObject.__s_unlocked)
                {
                    let state:number = RenderStateObject.GetRenderStateByName(stateName);
                    //trace("state: "+state+", stateName: "+stateName);
                    //if(RenderStateObject.Rstate.roState != state)
                    if(RenderStateObject.__s_state != state)
                    {
                        if(state > -1 && state < RenderStateObject.__s_statesLen)
                        {
                            RenderStateObject.__s_states[state].use();
            ...
            {
                RenderStateObject.__s_blendMode = -1;
            }
            static LockBlendMode(cullFaceMode:number):void
            {
                RenderStateObject.__s_blendMode = cullFaceMode;
            }
            static UnlockDepthTestMode():void
            {
                RenderStateObject.__s_depthTestMode = -1;
            }
            static LockDepthTestMode(depthTestMode:number):void
            {
                RenderStateObject.__s_depthTestMode = depthTestMode;
            }
            static Lock():void
            {
                RenderStateObject.__s_unlocked = false;
            }
            static Unlock():void
            {
                RenderStateObject.__s_unlocked = true;
            }
            static Reset():void
            {
                RenderStateObject.__s_state = -1;
            }
        }
