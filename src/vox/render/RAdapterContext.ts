...
class RAdapterContext implements IRAdapterContext {
	...
	initialize(rcuid: number, stage: IRenderStage3D, param: IRendererParam): void {
		...
		if (pdocument) {
			...
			this.m_viewEle.createViewEle(
				pdocument,
				this.autoSyncRenderBufferAndWindowSize,
				param.divW,
				param.divH,
				param.autoAttachingHtmlDoc,
				this.offscreenRenderEnabled
			);
	...
	private m_sizeWithTwo = true;
	private m_toBigger = true;
	...
	private normlizeViewSize(pw: number, ph: number, sync: boolean, k: number): number[] {
