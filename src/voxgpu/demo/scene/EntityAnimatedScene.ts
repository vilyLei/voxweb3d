...
class EntityAnimatedScene {
	...
	private initCamera(width: number, height: number): void {
		...
		cam.lookAtRH(new Vector3D(900.0, 900.0, 900.0), new Vector3D(0.0, 0.0, 0.0), camUpDirect);
		cam.setViewXY(0, 0);
	...
	initialize(canvas: HTMLCanvasElement): void {
		...
		this.texRes.buildDefaultTextures((texs: GPUTexture[]): void => {
	...
	private createEntity(materials: WGMaterial[]): void {
		...
		const rgd = gds[Math.round(Math.random() * (gds.length - 1))];
		...
		trans.setXYZ(-150 + k * 180, -150 + k * 180, 0.1 * k);
