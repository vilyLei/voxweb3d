...
export default class PingpongBlur {
     ...
     */
    constructor(rscene: IRendererScene, blurMode: number = 2) {
        ...
        if (this.m_wrapperTexs == null) {
            ...
            tex0.__$setRenderProxy(this.m_rscene.getRenderProxy());
            let tex1: WrapperTextureProxy = new WrapperTextureProxy(32, 32, true);
            tex1.__$setRenderProxy(this.m_rscene.getRenderProxy());
    ...
    public getTextureAt(index: number): RTTTextureProxy {
        if (this.m_texs[index] != null) {
            if ((this.m_fboWidth * this.m_fboHeight) > 0) {
                ...
                this.m_texs[index].setSize(this.m_rscene.getViewWidth(), this.m_rscene.getViewHeight());
            }
    ...
    run(srcProcessId: number = -1): void {
        ...
        if (this.m_drawEntity != null) {
            this.m_rscene.drawEntity(this.m_drawEntity, false, true);
        }
        if (srcProcessId >= 0) this.m_rscene.runAt(srcProcessId);
        // bluring
        let i: number = 0;
        for (; i < this.m_blurCount; ++i) {
            ...
            adapter.useFBO(true, true, false);
            if ((i % 2) == 0) {
                this.m_rscene.drawEntity(this.m_plane0, false, true);
            }
            else {
                this.m_rscene.drawEntity(this.m_plane1, false, true);
            }
        }
    ...
    destroy(): void {
        if (this.m_rscene != null) {
            this.m_rscene.removeEntity(this.m_plane0);
            this.m_rscene.removeEntity(this.m_plane1);
