...
import { IPathShapeCalcModel } from "./IPathShapeCalcModel";
import { SegmentData } from "../segment/SegmentData";

class HighwayShapeCalcModel implements IPathShapeCalcModel {
    ...
    initialize(pathEditor: PathCurveEditor): void {
        ...
        console.log("HighwayShapeCalcModel::initialize()......");
        if (this.m_initFlag) {
    ...
    build(roadWidthScale: number,): void {
        ...
        if (posTotal > 1) {
            ...
            // let posListLBase: Pos3D[] = this.m_pathTool.calcOnePosListByTVList(tvList, curvePosList, offsetXYZ, 0);
            // let posListR: Pos3D[] = this.m_pathTool.calcOnePosListByTVList(tvList, curvePosList, offsetXYZ, dis);

            let posListL: Pos3D[] = this.m_pathTool.calcOnePosListByTVAndDisList(tvList, curvePosList, offsetXYZ, radiusList, -1.0 * roadWidthScale, 0);
            let posListLBase: Pos3D[] = this.m_pathTool.calcOnePosListByTVList(tvList, curvePosList, offsetXYZ, 0);
            let posListR: Pos3D[] = this.m_pathTool.calcOnePosListByTVAndDisList(tvList, curvePosList, offsetXYZ, radiusList, 1.0 * roadWidthScale, 0);
            let dw: number = 8;
            let dh: number = 15;
            offsetXYZ.setXYZ(0, dh, 0);
            let posListLTop0: Pos3D[] = this.m_pathTool.calcOnePosListByOffset(posListL, offsetXYZ);
            let posListRTop0: Pos3D[] = this.m_pathTool.calcOnePosListByOffset(posListR, offsetXYZ);

            offsetXYZ.setXYZ(0.0, 0.0, 0.0);
            //let posListLTop1: Pos3D[] = this.m_pathTool.calcOnePosListByTVAndDisList(tvList, posListLTop0, offsetXYZ, radiusList, -1.0 * roadWidthScale, -dw);
            let posListLTop1: Pos3D[] = this.m_pathTool.calcOnePosListByTVList(tvList, posListLTop0, offsetXYZ, -dw);
            //let posListRTop1: Pos3D[] = this.m_pathTool.calcOnePosListByTVAndDisList(tvList, posListRTop0, offsetXYZ, radiusList, 1.0 * roadWidthScale, dw);
            let posListRTop1: Pos3D[] = this.m_pathTool.calcOnePosListByTVList(tvList, posListRTop0, offsetXYZ, dw);
            ...
            let table: Pos3D[][] = this.m_pathTool.calcTwoPosListByTVList(tvList, posListLTop1, posListRTop1, offsetXYZ, 0);
            lbList = table[0];
            rbList = table[1];
            
            let tableTop: Pos3D[][] = [posListL, posListLBase, posListR];
            //let tableBottom: Pos3D[][] = [posListR, rbList, lbList, posListL];
            let tableBottom: Pos3D[][] = [posListR, posListRTop0, posListRTop1, rbList, lbList, posListLTop1, posListLTop0, posListL];
