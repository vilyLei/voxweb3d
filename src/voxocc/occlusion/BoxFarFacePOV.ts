...
export namespace voxocc
{
    export namespace occlusion
    {
        ...
        export class BoxFarFacePOV implements ISpacePOV
        {
            constructor()
            {
            }
            ...
            private m_projNV2Total:number = 0;
            ...
            setParam(minV:Vector3D,maxV:Vector3D):void
            {
                if(this.m_pvList[0] == null)
                {
                    ...
                    this.m_faceList.push(
                        new BoxFace(0),new BoxFace(1),new BoxFace(2)
                        ,new BoxFace(3),new BoxFace(4),new BoxFace(5)
                        );
                }
                else
                {
                    ...
                    if(i >= this.m_projNVTotal)
                    {
                        ...
                        if((cullMask & SpaceCullingMasK.INNER_POV_PASS) == SpaceCullingMasK.INNER_POV_PASS)
                        {
                            ...
                            if(Vector3D.Distance(cv,this.m_centerv) < (pr + this.m_sphOcc.occRadius))
                            {
                                ...
                                {
                                    if((this.m_planeNVList2[i].dotProduct(cv) - this.m_planeDisList2[i]) > pr)
                                    {
                                        break;
                                    }
