...
import * as FogSphGeomFactorMaterialT from "../../../advancedDemo/depthFog2/material/FogSphGeomFactorMaterial";
...
import * as FogFBOManaT from "../../../advancedDemo/depthFog2/scene/FogFBOMana";
...
import FogSphGeomFactorMaterial = FogSphGeomFactorMaterialT.advancedDemo.depthFog2.material.FogSphGeomFactorMaterial;
import BoxSpaceMotioner = BoxSpaceMotionerT.voxmotion.primitive.BoxSpaceMotioner;
import FogFBOMana = FogFBOManaT.advancedDemo.depthFog2.scene.FogFBOMana;
...
export namespace advancedDemo
{
    export namespace depthFog2
    {
        ...
        export class SceneFogSph2
        {
            ...
            {
                //  let len:number = this.volumeSphs.length
                //  for(let i:number = 0; i < len; ++i)
                //  {
                //      if(index == i)
                //      {
                //          this.volumeSphs[i].setVisible(true);
                //      }
                //      else
                //      {
                //          this.volumeSphs[i].setVisible(false);
                //      }
                //  }
            }
            ...
            {
                console.log("SceneFogSph2::initialize()......");
                if(this.m_rc == null)
                {
                    this.m_rc = rc;
                    ...
                    for(i = 0; i < 130; ++i)
                    {
                        ...
                        //fogVolumeSph.setPosition(pv);
                        //this.m_rc.addEntity(fogVolumeSph,this.m_fogVolumeIndex);
                        //this.volumeSphs.push(fogVolumeSph);
                        motioner = new BoxSpaceMotioner();
                        motioner.setPosition(pv);
                        this.m_volMotioners.push(motioner);
                        pcolor = new Color4();
