...
export class FontTexCharTable {
	...
	getVtxFromChar(char: string, vtxFloatArr: Float32Array, offset: number = 0): void {
		let grid: FontTexCharGrid = this.m_gridMp.get(char);
		if (grid == null) grid = this.m_gridMp.get("?");
		let pos_arr: number[] = [0, 0, 0, grid.width, 0, 0, grid.width, grid.height, 0, 0, grid.height, 0];
...
export class FontTexDataBuilder {
	...
	private m_fontCharTable: FontTexCharTable = null;
...
class H5Text implements IH5Text {
	...
	private m_ctx2D: CanvasRenderingContext2D = null;
	private m_tex: IBytesTexture = null;
	private m_fontSize = 18;
	private m_fontCharTable = new FontTexCharTable();
	private m_fontTexDataBuilder = new FontTexDataBuilder();
	...
	initialize(
		...
		if (this.m_canvas == null) {
			...
			this.m_canvas = document.getElementById(canvas_id_name) as HTMLCanvasElement;
			if (this.m_canvas == null) {
			...
			//console.log("H5Text::initialize(), canvas_visible: "+canvas_visible);
			if (canvas_visible) {
	...
	createCharsTexFromStr(srcStr: string): void {
		...
		let rawBytes = new Uint8Array(dw * dh);
	...
	private createInitTexAndChars(): void {
		...
		let rawFontText: string = baseChars;
	...
	getTextureAt(index: number = 0): IBytesTexture {
		return this.m_tex;
	}
}
export { H5Text };
