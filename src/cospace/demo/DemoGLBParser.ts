...
/**
 * 通过加载到的GLB模型二进制数据，发送CTM资源解析任务给多线程数据处理系统，获取解析之后的CTM模型数据
 */
export class DemoGLBParser {
	...
	initialize(): void {
		...
		console.log("DemoGLBParser::initialize()...");
	...
	ctmParseFinish(model: GeometryModelDataType, url: string): void {
		...
		console.log("DemoGLBParser::ctmParseFinish(), model: ", model, ", url: ", url);
	}
