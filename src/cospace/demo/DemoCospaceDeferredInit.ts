 ...
 * 引擎数据/资源协同空间
 */
export class DemoCospaceDeferredInit {
	/**
	 * (引擎)数据协同中心实例
	 */
	...
	initialize(): void {
		console.log("DemoCospaceDeferredInit::initialize()...");
	...
	private loadPNGByCallback(url: string): void {
		...
		texture.getCPUDataByUrlAndCallback(
			...
			(unit: TextureDataUnit, status: number): void => {
				console.log("DemoCospaceDeferredInit::loadPNGByCallback(), texture data:", unit.data.imageDatas[0]);
				console.log("DemoCospaceDeferredInit::loadPNGByCallback(), texture des:", unit.data.desList[0]);
	...
	private loadDracoCallback(url: string): void {
		...
		geom.getCPUDataByUrlAndCallback(
			...
			(unit: GeometryDataUnit, status: number): void => {
				console.log("DemoCospaceDeferredInit::loadDracoCallback(), draco model data:", unit.data.models[0]);
				console.log("lossTime: ", unit.lossTime + " ms");
			},
			true
		);
	...
	private loadCtMAt(index: number): void {
		...
		this.m_cospace.geometry.getCPUDataByUrlAndCallback(
			...
			(unit: GeometryDataUnit, status: number): void => {
				...
				// console.log("DemoCospaceDeferredInit::loadCTMByCallback(), geometry model", unit.data.model);
	...
	private loadFBX(url: string): void {
		...
		let unit = this.m_cospace.geometry.getCPUDataByUrlAndCallback(
			...
			(unit: GeometryDataUnit, status: number): void => {
				...
				// console.log("DemoCospaceDeferredInit::loadFBX(), geometry models:", unit.data.models);
				// console.log("model: ", unit.data.model);
				// console.log("one res lossTime: ", unit.lossTime + " ms");
	...
	private loadCTMByCallback(): void {
		...
		geometry.getCPUDataByUrlAndCallback(
			...
			DataFormat.CTM,
			(unit: GeometryDataUnit, status: number): void => {
				console.log("DemoCospaceDeferredInit::loadCTMByCallback(), geometry model", unit.data.models[0]);
...
export default DemoCospaceDeferredInit;
