...
type StreamDataType = Uint16Array | Float32Array | Uint32Array;
type UVMapType = {
	name: string;
	filename: string;
	uv: Float32Array;
...
class InterleavedStream implements ICTMWStream {
...
class FileHeader {
	...
	constructor(stream: ICTMRStream) {
...
class CTM {
	...
	static InterleavedStream(data: StreamDataType, count: number): InterleavedStream {
	...
	static restoreVertices(vertices: Float32Array, grid: FileMG2Header, gridIndices: Uint16Array | Uint32Array, precision: number): void {
		let gridIdx,
			delta,
			x,
			y,
	...
	static restoreMap(map: StreamDataType, count: number, precision: number): void {
		let delta,
			value,
...
interface IReaderHeader {
	read(stream: ICTMRStream, body: CTMFileBody): void;
}
class ReaderRAW implements IReaderHeader {
	constructor() {}

	readAttrMaps(stream: ICTMRStream, attrMaps: AttrMapType[]): void {
		let i = 0;
		for (; i < attrMaps.length; ++i) {
			stream.readInt32(); //magic "ATTR"

			attrMaps[i].name = stream.readString();
			stream.readArrayFloat32(attrMaps[i].attr as Float32Array);
		}
	}
	readUVMaps(stream: ICTMRStream, uvMaps: UVMapType[]): void {
		let i = 0;
	...
	readNormals(stream: ICTMRStream, normals: Float32Array): void {
		stream.readInt32(); //magic "NORM"
		stream.readArrayFloat32(normals);
	}
	readVertices(stream: ICTMRStream, vertices: Float32Array): void {
		stream.readInt32(); //magic "VERT"
		stream.readArrayFloat32(vertices);
	}
	readIndices(stream: ICTMRStream, indices: Uint16Array | Uint32Array): void {
...
class ReaderMG2 implements IReaderHeader {
	...
	readGridIndices(stream: ICTMRStream, vertices: Float32Array) {
	...
	readNormals(stream: ICTMRStream, body: CTMFileBody): void {
		stream.readInt32(); //magic "NORM"
		stream.readInt32(); //packed size
...
class CTMStream implements ICTMRStream {
	readonly TWO_POW_MINUS23 = Math.pow(2, -23);
	readonly TWO_POW_MINUS126 = Math.pow(2, -126);
	/**
	 * uint8 array
	 */
	data: Uint8Array;
...
class CTMStringStream implements ICTMRStream {
