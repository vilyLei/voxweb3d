...
export class DemoLighting {
        ...
        if (this.m_rscene == null) {
            ...
            for(let i:number = 0; i < rn; ++i)
            {
                ...
                for(let j:number = 0; j < cn; ++j)
                {
                    ...
                    //let material:PBRLightingMaterial = this.materialBuild(0.0, 0.2, 1.0);
                    let material:PBRLightingMaterial = this.materialBuild(metallic, roughness, 1.3);
    ...
    private materialBuild(metallic: number, roughness: number, ao: number): PBRLightingMaterial
    {
