 ...
 * velocity controller
 */
export default class VelocityXZModule {
    ...
    setDirecXZ(dx: number, dz: number, force: boolean = false): void {

        if(this.spdv.getLengthSquared() > MathConst.MATH_MIN_POSITIVE) {
            this.spdv.normalize();
            this.m_prev.copyFrom(this.spdv);
        }
        if(force) {
            this.m_prev.setXYZ(0.0, 0.0, 0.0);
        }
    ...
    run(): void {
        ...
        this.spdv.x += this.m_curr.x * this.m_currvk;
        this.spdv.y += this.m_curr.y * this.m_currvk;
        this.spdv.z += this.m_curr.z * this.m_currvk;
