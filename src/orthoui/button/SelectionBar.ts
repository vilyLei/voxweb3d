...
export class SelectionBar {
	...
	initialize(
		ruisc: IRendererScene,
		barName: string = "select",
		select_name: string = "Yes",
		deselect_name: string = "No",
		btnSize: number = 64.0
	): void {
	...
	setXY(px: number, py: number, force: boolean = true): void {
		if (this.m_container) {
			...
			if (force) this.m_container.update();
		}
	}
	...
	private initBody(): void {
		...
		if (haveNameBt) {
			...
			this.nameButton.addEventListener(MouseEvent.MOUSE_OVER, this, this.overListener);
			this.nameButton.addEventListener(MouseEvent.MOUSE_OUT, this, this.outListener);
		}
		if (style) {
		...
		if ((style == null && this.bodyVisible) || (style && style.bodyVisible)) {
			...
			this.selectionButton.addEventListener(MouseEvent.MOUSE_OVER, this, this.overListener);
			this.selectionButton.addEventListener(MouseEvent.MOUSE_OUT, this, this.outListener);
		}
		if (style) {
	...
	private nameBtnMouseDown(evt: any): void {
		if (this.m_enabled) {
			if (this.updateStateEnabledByMouseEvt) {
	...
	updateStateEnabledByMouseEvt = true;
