vec3 getNormalFromMap(sampler2D nor_sampler, const in vec3 wpos, const in vec2 uv, const in vec3 nv)
{
    vec3 tangentNormal = VOX_Texture2D(nor_sampler, uv).xyz * 2.0 - 1.0;
