...
class ClockEntity {
	...
	initialize(rc: IRendererScene, radius: number, processId = 0): void {
		if (this.m_rc == null && rc) {
			...
			let mainContainer = (this.m_body = new RenderableEntityContainer());
			this.m_rc.addEntity(mainContainer, processId);

			let bodyContainer = new RenderableEntityContainer();
			...
			let texContainer = this.createText(radius, mainContainer);
	...
	private createText(radius: number, mainContainer: IDisplayEntityContainer): IDisplayEntityContainer {
...
export class DemoEntityContainerRender2 {
	...
	private m_cet: ClockEntity = null;
	initialize(): void {
		console.log("DemoEntityContainerRender2::initialize()......");
		if (this.m_rscene == null) {
			...
			let c = clock.getContainer();
			c.setXYZ(-200, 0, 200);

			clock = new ClockEntity();
			clock.initialize(this.m_rscene, 100.0);
			c = clock.getContainer();
			c.setXYZ(200, 0, -200);
			this.m_cet = clock;
		}
	}
...
export default DemoEntityContainerRender2;
