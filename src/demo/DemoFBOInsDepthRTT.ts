...
import DepRTTShowMaterial from "./material/DepRTTShowMaterial";


export class DemoFBOInsDepthRTT {
    ...
    initialize(): void {
        if (this.m_rscene == null) {
            console.log("DemoFBOInsDepthRTT::initialize()......");
    ...
    private buildRTT(): void {
        ...
        plane.initializeXOZ(-2000.0, -150.0, 4000.0, 300.0, [tex0]);
        ...
        fboIns.setRenderToDepthTextureAt(0, 1);                             // apply the second rtt texture, and apply the fbo framebuffer color attachment 1
        fboIns.setRProcessIDList([0], false);
        fboIns.setAutoRunning( true );

        let scrPlane = new Plane3DEntity();
		let material = new DepRTTShowMaterial();   
        material.setTextureList([fboIns.getRTTAt(0), fboIns.getRTTAt(1)]);
		scrPlane.setMaterial(material);
		scrPlane.initializeXOY(-1.0, -1.0, 2.0, 2.0);
		this.m_rscene.addEntity(scrPlane, 1);
    }
}
export default DemoFBOInsDepthRTT;